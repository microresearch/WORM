...
const uint8_t tmsEnergy[0x10] = {0, 0, 1, 1, 2, 3, 5, 7, 10, 15, 21, 30, 43, 61, 86, 0};

//const uint8_t tmsEnergy[0x10] = {0,  1,  2,  3,  4,  6,  8, 11, 16, 23, 33, 47, 63, 85,114, 0 };
...
void lpc_update_coeffs(void)
{
	...
	//	fprintf(stderr, "OFF: %d\n", ptrAddr-xxx);
		...
		else
		{
			...
			//			ptrAddr++;
			//									nextPeriod = tmsPeriod5220[lpc_getBits(6)]; // TEST for 5110
	...
	*/
	...
	if(synthPeriod)
	{
		...
		if (periodCounter < CHIRP_SIZE)
		  ulpc[10] = ((chirp[periodCounter]) * (uint32_t) synthEnergy) >> 8; // TESTY
		else
			ulpc[10] = 0;
	}
	else
