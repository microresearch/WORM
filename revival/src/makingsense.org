...
bends and worms these key speech synthesis algorithms across 64 modes
...
ERD/WORM ships with a golden Riso-printed full manual in a limited
edition of 100. The first batch of 25 units is now available for
tracked shipping. The next batch will be available in February 2018.

Order here: http://www.1010.co.uk/org/ERD.html

///AND...

A related workshop exploring wormy speech synthesis and building your
very own touch-based voice synthesizer (though not the ERD/WORM) will
take place in London at the end of next week:
...
last century. Six key speech cores are simulated with a vocabulary of
over 4000 words and phrases, including basic allophones or phonemes
which can be used to build new phrases as well as a Text to Speech
engine for advanced textual constructions. A further set of COMPOST
modes allows for sample-based re-working of constructed and wormed
words and phrases.
...
ERD/WORM ships with a golden Riso-printed full manual in a limited
edition of 100. The first batch of 25 units is now available for
immediate tracked shipping. The next batch will be available in
February 2018.

Pricing: 265 euros with free, tracked worldwide shipping. 24 now in stock of first batch.

**** Sounds like:

<iframe width="100%" height="450" scrolling="no" frameborder="no" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/playlists/332275862&amp;color=%23130701&amp;auto_play=false&amp;hide_related=false&amp;show_comments=true&amp;show_user=true&amp;show_reposts=false&amp;show_teaser=true"></iframe>
...
Manual, code and design files at: http://1010.co.uk/ErdWormOnlinewhitemanual.pdf and https://github.com/microresearch/WORM
...
28/11 HW programmed and tested TOTAL: 27

TO re-program as some small changes 1/12/// DONE ALL 27

* latest/latest

// 5/12 - check compostmodef start/trigger code? TEST!DONE_COMPLETED

// code checks - votraxX, samX, digitalkerX, rsynthX/klatt=rsynth-2.0-port/simpleklattX/nvpX

// 29/11 - newvotrax: mainclock in rawparam as no newsay in votraxparamer and also changes in sp0256bender - TEST!

// 27/11 - trigger is fine on exy and normal, compost is fine, reduced float on modesDONE

// 25/11 - changes to trigger -> test on exy and normal modes - TEST!DONE


- final sanity check of all modes - DONE

- check code-> audio.cDONE

// 28/11 - re-check sp0256bender=27 and votraxparamer=34 as added null newsay for compostDONE
// check 53/54 clicky????CHECKED - recheck sanity

// re-check compostDONE

check HW 14.31 several hours fine

// added clear audio buffer in main
...
// changed votrax sample countings -> CHECK CODE FROM HERE ON
...
&43samxyer=TRIGGER_WEIRD/fixed wrong newsayDONE, &44samparamer, &45sambender, 

&46digitalker,
&47digitalker_sing, &48digitalker_bendpitchvals, 

&49rsynthy,
&50rsynthelm, &51rsynthsingle, &52rsynthysing,  ADJUSTED_VOLUMES

//rsynth-2.0-port
&53klatter, 53/54 clicky?
...
// some changes to nvp interpol/old/new framesDONE-tested + changed voice more breathynow
...
26/10 - small fixes to tms raw to check?

////

DONE-triggerX-changed so trigger and triggered don't run twice, compostX

////


- check all hardware before release - reprog first set of 3...


** 18-19/10

- check TTS and triggers on modesDONE

- check new reset of states in sam... - REMOVED as problem was in resets... FIXED

- checked all resets - fixed in simpleklattDONE-TEST!, in klattTEST, in rsynthTEST, in votraxTEST-DONE/TESTED

checked nvp, tms, digi, sp0256 in code

- check fast triggering and changing between modes - re-check all modes! 0->61DONE

NOTES: 17,18 - tms is noisy (but not to do with trigger)

RE_CHECK:
29-votraxTTS trigger???-> votrax_retriggerTTS-FIXED
///

53-klatt trigger???-> klatt_newsay // or is 54?fine!

58-simpleklatt trigger-> simpleklatt_newsay OK!

////

- full modes workout (2x SIR?), volume and other tweaks and fixes (first TMS volume leave as is)...

// changed newvotrax.c volume mult

- vol/limiting Q? leave as is now

/// check through tms noisiness - we need to re-work raw ones (5100-7 also)! checked-DONE/ no changes...

- should these frames have a length? length would need to be an extra parameter - or just use trigger...
- we do not use any unvoiced here...

&8tmsraw5200er*CLICKS*,
&9tmsraw5220er*CLICKS*, 

&10tmsbend5100erX, &11tmsbend5200erX, &12tmsbend5200erxCLICKY
...
*25/10 - changed limit volume back to 32000 ///*

- final sanity check of all modes, raw modes// 
...
- limitings in samplerate.cDONE
- glitches when changing between sam modes and also on too fast re-trigger so added limit to how fast can do newsay TEST!
