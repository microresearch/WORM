...
in nvp there is no skew but is in klsyn and also rsynth:
...
ideally, will be the source signal as if the person had no vocal tract
at all.

from: http://savethevowels.org/praat/html/UsingPraatforLinguisticResearch.html
...
- PORTED in latest worm code - but is glitchy
