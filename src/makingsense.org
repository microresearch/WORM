...
*TODO: SAM selected vocab, digitalker ROM code, logarithmic bends, what is knob orientation for repeat/length of phoneme, 
...
test_speakIpa.py - test first of all:
...
const worm[] =  { { 46, 110.190511588, 101.54464594, 42.0, 28.0}, { 6, 101.54464594, 83.3883280795, 88.2, 35.0}, { 41, 83.3883280795, 74.7424624317, 42.0, 14.0}};

const code[] =  { { 49, { 40, 110.190511588, 110.190511588, 6.0, 0.001}, { 49, { 32, 110.190511588, 93.077660271, 84.0, 14.0}, { 20, 93.077660271, 75.9648089544, 84.0, 14.0}, { 49, { 8, 75.9648089544, 74.7424624317, 6.0, 0.001}};

const raod[] =  { { 47, 110.190511588, 105.227784706, 42.0, 28.0}, { 36, 105.227784706, 95.3023309422, 84.0, 35.0}, { 10, 95.3023309422, 85.3768771785, 84.0, 14.0}, { 4, 85.3768771785, 75.4514234149, 84.0, 14.0}, { 49, { 8, 75.4514234149, 74.7424624317, 6.0, 0.001}};

*mistake in last 2 with 49!*
...
TODO: singmode, pitch changes at start of phrase or all through (but we need to keep consistent length)

also maybe vocab for klatt as fixed test_elms so then we can bend
more *TODO: how did we generate the first test_elm?: /root/projects/ERD_modules/worm/docs/rsynth-2-un/rsynth-2.0: so set up for bulk*
...
- generate test_elm vocab lists for fullklatt: /root/projects/ERD_modules/worm/docs/rsynth-2-un/rsynth-2.0: so set up for bulk
...
//  PhonemeToWaveData(eng2phonemeOUT, eng2phonemeOUT_ptr, 0);



- work over all of klatt
- Q of len-1 for input in TTS/parse.c?
...
12. compost as last modes - startX, endY in compost buffer and Z? Z as last mode
    ...
    oldX.Y for example. compost as //wormed// wavetable. speed in middle - left-backwards/right-forwards
