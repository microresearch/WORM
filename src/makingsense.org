...
- sp0256 vocab-splitDONE and testTODO - 2 banksTEST!

- Y,Z to add = pitch and length - length as repeat or as audio.c sample length before a newsay?

- XYZ for each in co-ordinate and bends (x sel/pitch, y pitch/other, z- length of sample)
and eg. for sp0256: // selx,y,z-> x-axis for amp, 12x filt-coef,
on/off for noise, y for value, z for period/pitch! how we do that
mapping 

- port sp0256_lapraw1 to ARM-DONE-> TEST!

- raw2 - code version on lap for sp0256: Instructions: http://spatula-city.org/~im14u2c/intv/tech/sp0256_instr_set.html

-- we need buffer (how long?) for xy entry as ROM, no jumps etc...

- AND add bends which shift the ROM values! 

*- whether we do changes to x,y,z in audio.c on timer or in sp0256?*

- finalise all sp0256 and use as model - how many modes now??:

sp0256TTS, sp0256vocabone, sp0256vocabtwo, sp0256_12, sp0256_19, sp0256rawone + 2nd raw, bending of roms, other excitations = 8+
...
- TTS votrax port and test - we need to fix x, y and z also! - x-sel,
  y-inflection/pitch, z-length - decide on this for all basics and integrate
  with audio.c model
...
- also question of phoneme probability mapping for all chipsets - recap how we did this for klatt or do by hand... - TTS and probability-NLTK?
...
- sp0256 do we need to implement  m_silent   if (m_silent && m_filt.rpt <= 0)
