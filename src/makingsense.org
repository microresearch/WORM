...
- TEST all klatt vocabs and fix nvp silence - *- for nvp.c we need frame 49 as silent frame - NEARLY DONE - just need to set volumes*

- check sam selected vocab
- TODO for tubes: singing mode (straight freq and length), raw xy mode (16 params + reset volume thing in that)
- digitalker raw mode
