...
- FIX ADC extents! - see newsaybankone - works on new hardware but would *need to test across other HW versions* and tune the filter coeffs
...
- refine and test triggers in all sp0256 modes
...
- add bend mode which shifts the ROM values ->
...
sp0256_12+*JOIN+sp0256_19, sp0256rawone, sp0256rawtwo, bending of
roms, other excitations = 9+
...
- add bend mode which shifts the ROM values -> which roms? AL2/and/or/12-19? - x/y bend and z select! 

** 1/2 TODO

- sp0256 - test triggers/ all modes - try different filter coeffs... (0.01f)

- sp0256 AL2 - how to map allophones as all pauses are at one end?

- ported sp0256_lapraw1, raw2 to ARM-DONE-> TEST BASICS!DONE- raw2 fixed, raw1 fixed but both need tweaking for speeds and so on
...
- new hardware tests for ADC extents
