...
- re-look at sp0256bend and finalise all first modes
...
*test each modeXX and clean up votrax codeDONE, clean up audio.c for first modes*

///

- policy for selx so don't have to repeat eg. real extent etc - struct for each exy mode or?

*TODO- maybe put all selects on Z so leaves x/y*
