...
- start on Klatt - define all klatt sub-types (eg. nvp, simpleklatt, TTS there etc.)
...
*TODO: SAM selected vocab, digitalker ROM code, logarithmic bends, repeat knob orientation, Klatt breakdown:*

1-simpleklatt.c+parwave= raw frames, nvp.c -CLEANUP

2-nvp.c= phoneme frames

3-fullklatt/holmes.c=klatt_get_sample // test_elm list of phonemes // single phoneme with transition

4-TTS for fullklatt?: TODO // would need to break down all from wav_file >??? - 	xlate_file();	PhonemeToWaveData();

- or we use other TTS/mapped and figure out phoneme transitions: see ratts?

- what we parameters we have also (f0, excitation, parallel/cascade etc.. *LIST with all klatt notes* )
