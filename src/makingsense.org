...
*- what can be varies with each of klatt voices: and how - on basis of fixed voice parameters?*
...
for rsynth: 2005 - python speakyr.py using vocab in /root/test - what is that VOCAB?

** 31/3+

*- still check all length long->short (fixed for all klatts and few others)* DONE

*- digitalker ROM modes - ssr1, ssr2, scorpion - how many phrases?*
*- in sam.c sam_newsay_xy(void){ // TODO for extended Vocab - is just basic select now*

*- fix remaining modes = tubes, compost and start on wormings*
*- all vocabs for nvp, klatt, rsynthy*
*- all klatt extra voices as above - easy way of testing these?*

*** tubes.c - question of interpolation, data, newsay - lots more TODO?

trying to extract data from diphones.xml in postures_demo.py we have:
...
- put back in missing TMS vocabs, changemode triggering, all vocabs and check TESTING/TODO marked code/cleanup
- test/document each mode
- manual outline

- HW tests: test programming, finish build, test all (maybe code one
