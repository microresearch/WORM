...
[0] is CMD = 1, 2, 3, 5
[1] is length
[2] and [3] are pitch [2] as pointer (to a pitch env?) (if WCMD_PITCH is CMD = 2) or amplitude pointer (1) or peak_t pointer -> and then we need to record this as:

so we can print/store all these values... TEST!
...
- what causes the variation (other than amp and filter?)
