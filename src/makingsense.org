...
- end of phoneme in element list: 1,6,0 twice???? - to keep always in elements so don't overwrite?
...
nvp vocab sounds a bit strange (added length and pitchinc) but maybe try with larger vocab!
nvp also seems quite breathy

- all other rsynths: note also opsynth in ... - which can we use for testing on lap?

- what we need to do to complete nvp/klatt - per sample bends, breathiness, port of opsynth (below)
...
// port in progress: compiles but we need to break down frame:DONE 

TO FIX: init, where do we set speaker
