...
- check clean extent for 64 modes - should be ok!

- tweak speeds for each mode? - where normal speed should be for each mode?

- new bends/TMS modes in tms5200x.h to incorporate in audio.c, TEST and delect/add new vocabs

int16_t tms_newsay_specific(u8 whichbank); is new vocab selection make
sure is matched in audio.c for each of new modes and add new modes for new vocabs for each of these bends

- check all TMS alphon code:     now tms_newsay_specific(1); // TODO this will change with full vocab to tms_newsay_specific(37)
...
5. SAM -> TTS, vocab(ported already), raw, low pass filtered and not filtered
