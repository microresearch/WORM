...
- add 5220 bend mode (simply copy but when we are happy with X/Y)

- extra subsections of vocabs with both bends TODO - which vocabs? - allphons we have already and ...

- X/Y bends/add-subs on subsection of vocab TODO - which vocabs? - allphons and ...

++ bends of tables on specific vocabs. tables are: energy, pitch, LPC and chirp


*** GENERAL

 -  test and clean code for each mode following:


 - exponential pitch - where do we need this (I guess on any pitch bend) - lookup as we always have reduced scale..
...
TMS: from -50 to +50 // if (m_pitch_count >= (m_current_pitch+((_selx*100.0f)-50))) m_pitch_count = 0; // TEST - pitch bend
 ...
 - still not happy with x/y selecting protocal
