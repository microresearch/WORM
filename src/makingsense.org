...
//- extents/first modes/start on TMS
////
...
- policy for extents (how far we go beyond)DONE
*// some kind of extent test code = clear vocab as best - for say 150 we need to +4* - test on other hardwareDONE

also code for ROM grouping: in lpc_say.c start to catalogue

- finalise all first modes 256/votrax so far

///

*- TMS: test coeff switching - do we need to reset anything - TEST?*

- re-start on TMS and bends of_line593commented pitch bend//TMS TTS TODO ->

bends: energy, pitch, bends to tables, length of thing, using the
wrong coeff set (only across 5200 and 5100 variants itself)

- also do we do phrase/bank Z/Y or have list of modes for each bank (and so with extra bend)

phrase and bank with different bends and some chosen vocabs for multi-bends and other extras

what does TALKO bend?
...
for each vocab - generalise for each vocab -> pointer or?

- klatt notes
- other modes and start manual for each mode
...
14. extra modes for proposes RAVEN - excitations/wavetables
...
X- text xaxis
Y- select ascii character for TTS
Z- period/pitch
...
Y- select ascii character for TTS
Z-bend pitch
TRIGGER-reset to start/redo TTS
