...
- test sp0256 bend mode - working in principle but is only update at
  end when we call micro? - alt bend of all freq parameters
  (interpolation question?) DONE - makes more sense than raw1 so maybe
  ditch raw1 - also question of freeze/toggle on trigger in but we use trigger for newsay!
...
- sp0256 AL2 - how to re=map allophones as all pauses are at one end
  and we need to reverse - simply reverse - RE_TEST!

////.......

- sp0256 - test triggers/ all modes
...
- tweak and clean all sp0256 for release
