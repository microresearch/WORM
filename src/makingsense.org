...
- inverse filtering levels and noise
- check speed of inverse_filter -> sp0256
- x/y program resolved for all
- policy for extents (how far we go beyond and does it wrap or something in any cases?)

- re-look at sp0256bend - moved changes into sample loop. toggle is still new phrase
...
- start on TMS and bends of//TMS TTS TODO ->

- TMS TTS code/mappings -> DONE testing in lpc_say but sounds
  choppy???// also signal end of with 255->DONE - testing in lpc_say -
  but may not be so in TMS5220 code TEST!

- TMS: test coeff switching struct-in progress, grouping of TMS vocabs and clear chipsets marked for arm mame code:
