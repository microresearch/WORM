...
- removed extra TTS mode for SAM and placed change for spaces in parse.c RE_CHECK thisDONE

- shifted multiple klatt inits to main.c
...
- moved audio_buffer to ccmdata and still crash

seems to relate to compost???? (tests fine without compost - but test
again to rule out any connection TESTED- now also crashes without
compost)

re-TESTING so far modes: 5X, all TTS modes=8X, 17X, 30X, 25X

raw and bend/xy modes:
...
|        |         | sp02561219er     |       |          |         |               | gaps in vocab                    |
|        |         | sp0256bender     |       |          |         |               | noisy at first and gaps in vocab |
