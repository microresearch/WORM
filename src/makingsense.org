* for march 20 spring equinox release
...
- TMS TTS code/mappings -> DONE testing in lpc_say but sounds
  choppy???// also signal end of with 255->DONE - testing in lpc_say -
  but may not be so in TMS5220 code TEST!
...
/// - TODO 3/3 draft these out to inc into audio.c and test

2-  full tms with length bend - bend length on selx TEST
+ 8 bit and high quality mode for full tms as mode 1 (as we have it is undefined - defined as 12 bits now) TEST

+ also calculations for shorter lengths in interpol scheme - TEST just with define for now
/////
3+- subsections of vocabs with both bends TODO - which vocabs?
4+- X/Y bends on subsection of vocab TODO - which vocabs?

5+- raw raw mode TODO - starting on parse_frame_raw - and keep pitch
bend on selx as these are just frame bends - *should be for 2 different coeff sets - 5100, 5200 and 5220*



- how bends are applied for sp0256 and votrax (constant, per phonetic element)

*maybe change all TTS so phoneme select is on Z and x axis is Y so we can keep x as bend* - TODO for sp0256 and votraxDONE
...
- look at latest tms5220.cpp and m_zpar, old_zpar code? more for interpolation?
...
** basic modes to do with suggested extensions - say 48 modes
...
// above close to done-TMS to finish

//////////////////

3. digitalker
5. SAM -> TTS, vocab(ported already), raw, filtered and unfiltered
...
say 6x7=42+ // 7 or 8 main speech engines

////////////////////////////

// or we leave from below and any LPC work for RAVEN which could have 2 inputs?
...
*** withLPC - two varieties of sp0256 within (one with residual, one without)

X- period/pitch
Y- repeat/length
Z- select phrase - rom12+19
TRIGGER- UNUSED as we have carrier input
...
Y- length of phoneme
Z- select phoneme
TRIGGER-reset to start of phoneme
