...
*** voices/bends in klatt/tbe modes
...
- check voices on rsynth and nvpIN PROCESS, how to approach klatt voices?

- straight nvp does crash/go into silence -> FIXED bug in NVP!

- is raspiness at higher speeds property of sinc or overprocessing -
  try with simple wavetable/singles-code is there, how to speed up - delay
  length/width? - re-check now as delay_size was too low, try sinc table with say 8*8 - still no JOY!

- revert xy samplerate to simple linear interpolation which we will stick with!

- for wrapping in tms - newsay_specific if we want to add extra bend vocab modes?

*- need frozen compost mode where buffer stays the way it is*
