...
//fine up to 64 but above we lose the end with +1 - from mutable clouds/warps
...
TODO - Y,Z to add = pitch and length - length as repeat or as audio.c sample length before a newsay?

- AND add bends which shift the ROM values! 

*- whether we do changes to x,y,z in audio.c on timer or in sp0256? - in audio.c*
...
- for 1/2 - finalise ADC/ test triggers/ all sp0256 modes/ new working format?
