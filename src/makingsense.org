...
- check modes as above - all tms first=0->61:DONE , check rsynth elm
  ...
  bug in rsynthysing, less glitchy version of sp0256bender 

- question if to reduce digitalker and nvp volume - we need to compare all volumes?
- simpleklatt could be somehow improved
- nvp distorts quite badly on speed - but is maybe ok

- FULL test is fine but glitch still on  

IN CODE:
- check logic of tms mode and all mode changes
- check each set of exy extents for each mode//also all TTS length
- check logspeed/pitch for each mode
- check testy comments etc.

////

- start testing individual modes and whole thing again
