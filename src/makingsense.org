...
*- in sam.c sam_newsay_xy(void) // TODO for extended Vocab - is just basic select now->LAP*
*- remaining modes = tubes, compost and start on wormings*
*- all vocabs for nvp, klatt, rsynthy ->LAP*
...
- do we need to face up to interpolation - try various strategies -
  see samplerate.c (wrap up with function pointer to newsay and
  getsample plus how we handle the rest of stuff??)
