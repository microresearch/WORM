...
1- NON - as is too much timing based!
robo: arduino = 8 bit
...
[4-
Tiny Speech Synth: C code - TinySynth.h 16 bit -> tiny.c]

[5-
/afs/athena/astaff/project/phones/Speak emulates SPO256 - not really ]
...
*do away with END and use as 3rd parameter - eg. in phonemes do we even use 2nd par?*
...
- using encoder - calc.m and romgen from talkie with freemat - use 8 KHz 16 bit mono recordings to encode words for lpc.c
...
- wormlpc (but there in analysis phase also see sc live stuff),
  wormvoice simulation, how to do crossfading - see borsboom vocoder
  also for overlap
...
To see: braids code only for worming, Borsboom vocoder to port to static memory allocation, LPC live analysis = 

vocoder added and compiles (doubles to floats and lots of commented out) but long way off
