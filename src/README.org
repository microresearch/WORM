...
- talkie/lpc.c uses TI_0280_PATENT_CHIRP rather than LATER_CHIRP
  (tms5110r.hxx all refs), pitch is close but bit off, how do coefefs
...
- SAM, sam.c working on lap (sam_lap: gcc sam_lap.c render_lap.c -o
  ...
  we lookup into this, how and when we can change pitch etc, what else
  can be changed, speed and interpol in audio.c, number of bits we use is correct
  at 4 bits, timetableindex (work with interpol) in render.c in:
...
- 5110 - lpc_reader changed for 5110 renders  ./lpcr 0 0 /root/Downloads/speaky/tmc0351n2l.vsm > /dev/dsp  from 1024+

looking at newchirp.. compare for 5200! switch on banks?
