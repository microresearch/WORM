...
- digitalker to arm and extra vocab

- latest wvocoder tests? WORKING - *TODO: formantshift to test and our own crossing, envelope stuff as param, noise and pitch/type of wave*

- nvp.c tests and above... new mode as only move on next phoneme when is triggered

- singer.scm re-check if possible

- final work plan and notes below

* important notes

*- re-appraise all notes/progress!* WED NEXT WEEK! // go over full todo and place somewhere clear here (eg. channel
vocoder what needs doing, all generators and each one) below in notes

- all vocabs don't fit in FLASH so we need to trim here and/or use CCMData - how much we have? also some issues with CCMDATA?

- generators WITHOUT SPEED for full size not individual samples, and return void so not pass back if not needed
...
- wormings and grids of params HOW?

- what notes/code is for RAVEN?
...
- for TMS code we can have banks/vocabs/wordlists which use 5100, 5200 of 5220 coeffs/settings TODO!

- 5100 voacb to clean up and test//5220 to test if it makes differences in a good way
...
- started with digitalker.c... where are ROMS we have some, TESTED.DONE on lap
  samplerate is 9000 or so and we have repeats and new_say *TO_TEST ON ARM*
...
*working but need to do generation of new parameters and interpolation, also check audio rate interpolation which we dont use*
...
** 8.5 nvp.c looking into

TODO: duration and interpol?, pitch bending, TEST!
...
** 8.6 - nvp.c new mode as only move on next phoneme when is triggered
...
** XX+ plus - worming through wavetables to TEST TODO! - worm wavetable for excitation with filters - for RAVEN code

** XX+ any morphing of lpcanalysis code?

** XX+ various: klatt single phonemes, raw sp0256 coding, audio buffer, some low passes of above - so maybe 32 modes
