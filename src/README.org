...
changes to split and dump these speaknspell codes and maybe find more!

looking at newchirp.. compare for 5200! switch these differences on banks? - newchirp and newperiod -> lpc_say

morph chirp/pitch codes with wormings

also INTERP_SHIFT?????

[[file:~/mame/src/devices/sound/tms5220.cpp::m_current_energy%20%3D%20(m_current_energy%20%2B%20(((m_coeff->energytable%5Bm_new_frame_energy_idx%5D%20-%20m_current_energy)*(1-inhibit_state))%20INTERP_SHIFT))*(1-m_zpar)%3B][file:~/mame/src/devices/sound/tms5220.cpp::m_current_energy = (m_current_energy + (((m_coeff->energytable[m_new_frame_energy_idx] - m_current_energy)*(1-inhibit_state)) INTERP_SHIFT))*(1-m_zpar);]]

shift to new mame/mess code for tms????

how filter works in talkie code for coeffs???? see: https://github.com/going-digital/Talkie/issues/6

try coeff code only substitute for talkie...,,, also what about INTERP_SHIFT - RE_DO lpc_say with mame/mess code

*wormed morphings of pitch and chirp tables*

*- maybe wrap up code we have so far-.trm,sp0256,klatt,rawklatt,sam - 5 modes and lap tests first*
...
- filter on interpolation and check interpolation???

- what and how repeats function for TMS and SP0256

- check CVs/DONE - PCB->check/orderTODO-DONE

- wormings - plot first on PC-DONE
...
subdivide or get rid of say talko as is duplicates...
...
- SELX is phoneme, SELY->pos, SELZ->length/or/stress/pitch and SPEED=dur of each or keep speed as speed!
...
how what we have differs from: https://github.com/s-macke/SAM/blob/master/src/main.c ??? *see above*

reciter which changes text to phonemes is difference

how to select phonemes and stress etc...
...
cleanlyOK, new ROMS-BELOW_TODO, parameters and bends=PITCH_DONE, port
into generatorDONE, text to speech NRL->retroTTS/phoneme.c
...
time phonemes, samplerate: http://www.cpcwiki.eu/index.php/SP0256_Allophones ours are short but we can run slower no? test again ???

10k samplerate?

list/find roms: *we have AL2 which is generic allophones

others dumped as HEX in LPC dir moved to sporoms
...
how is 5110 differing from 5220? check bits for pitch and tables :

5 pitch bits rather than 6 for 5200
