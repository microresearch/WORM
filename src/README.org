...
- all generators write to audio_buffer which is re-used

- all vocabs don't fit in FLASH so we need to trim here and/or use
  CCMData - how much we have? also some issues with CCMDATA? - was too
  little flash in stm32_flash.ld specified so now fixed but some commented out to speed up load

define these vocabs and differentiate: vocabs based on phonemes (SAM
and sp0256), and those on code (also sp0256, talkie and associated TMS series)

- interpolation/filters? example = tube.c, warps and samplerate.c/perry cook -> not sure if is worthwhile
...
- on HW resolve where we have the most change from external CV=FAR
  ...
  we need to invert, for SPEED not but for other params then maybe makes sense for PITCH say
...
*TODO: try out changed voice code, also some phoneme glitch somewhere in interpolation of parameters perhaps and we need to order phonemes*
