...
- interpol wavetable as source

- check klatt params

- try LPCraw with other filter models and also pitch track from autocorrelation

- extra worms

- physical models of raven

- own primitive wormy synthesis model as suggested by votrax, dirty synth above...

* TODO from 1 AUG:

- pitch and wavetable on trigger (SELZ as we use incoming audio) for LPC source and try this with other filters

- ntubes injection of white noise. LPF on reflection. how close to use for singer.scm model

- describe all modes and raven work

- extra types of worm

- basic wormed wavetables and 2d wormed wavetables

- text to speech/phoneme and re-mapping of this for sp0256 and SAM...
...
- generation of say raven wavetables using LPC/inverse filtering - lpcforlap.c
...
how is implemented in braids: - *LEAVE FOR RAVEN or NOT?*
...
but also wt_map array


->>>>>compare these synthetic (not source/filter) approaches for abstraction out into worm.c:
...
- see paper notes for abstracting out of excitation and filter for
  ...
  use input as excitation then we need to *live inverse filter using LPC: TODO how 0 lpcanalysis sc code to try*
...
- worm wavetable-2d?-> how is implemented in braids: - *LEAVE FOR RAVEN or NOT?*
...
** XX+ plus - worming through wavetables to TEST TODO! - worm wavetable for excitation with filters - for RAVEN code or HERE?
...
** for LPC/inverse filtering

- import sound file -raw as in open or wav as in lonmg sound file and then extract sound
