...
- for testroms convert all to const etc,, place in banks and test
...
- worm_movement.c first lap tests ok but need to think on how is
  applied to parameter lists first, how would worm wavetable work as
  one dimension?

- LPC/5220 random parameter tests done in lpc_say.c

- TODO: redo alpiner, buck rogers, star trek, mash
...
- BRING IN larger vocab from:   /root/projects/earthvoice2/Talkie-LPCcode/Talkie/examples DONEorNOT? DONE
...
- knobs end/start defined - far right is 0 which is GND, far left is -10V ref which doesn't hit the top! MEasure???

21/6 - with >>6 (so to 6 bits=64) we have around 0-(58-60) - either
try with resistor value or make up for this with 58-x and wraparound?

no great need to invert...

organise vocab banks around this??? tricky

why is so as we only get up to 3700 instead of 4096... = around 2.98 volts // 3 volts (mcp6002 will go to 0.025v of 3.3v rail = 3.275!)
...
- check CVs/DONE - PCB->check/orderTODO

- wormings - plot first on PC-DONE, worm wavetable

///////

- finish LPC vocabulary - refine/clean up

- new sp0256-DONE and vocabTODO - report on both TMS and sp0256 ROMs // [digitalk/s14001a?]
...
- cleanup new vocabulary// seems okay

 and groupings of -> now in files but we need to define banks of 64 (how many, how much memory, macro)
 
- banks of 64 words/phrases SELX/SELY, SELZ is nextperiod=pitch

- pausing/breaks/repeat

singing TI? tisingarc.txt

vocab notes from 9/6+ and LPC above to check
...
how what we have differs from: https://github.com/s-macke/SAM/blob/master/src/main.c ???
