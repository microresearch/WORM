...
- tube.c - crahsing on init or some clash somewhere????
...
talkie and different tables/5100/52xx/sam single phonemes/low pass variants and change the low pass

/////EVENINGS:

- new 5100 roms below to decode and clean up all 5100 roms

- sp0256 vocabulary/dictionary/text to speechL NRL see below and other rombanks
...
- ditch or not channel vocoder and move to port/test warps and maybe snokoder with arm_fft

////

on HW resolve where we have the most change from external CV=FAR LEFT - zero
should be far right which has NO addition of CV - so do we need to invert no?
...
check follower in warps vocoder, followers in effect.c are a bit primitive...
...
SELX: choice/freq of excitation/carrier, SELY: envelope skew/delay?, SELZ: morph channels
...
into generatorDONE, text to speech NRL->retroTTS/and/phoneme.c:
